...
**Challenge question 2 (related to example 6).** If a random variable $$X$$ is defined on $$\Z^+$$ such that $$\mathbb P(X = n) = \mathbb P(A_n)$$, what is the well-known distribution of $$X$$? Thus, what should $$\mathbb P$$ assign to this event $$A_n$$? What is the probability of any individual sequence $$(x_1,x_2,...)$$ of tosses? Is this a contradiction? (*Hint:* for the last part, consider countable additivity of $$\mathbb P$$. In particular, what is $$|A_n|$$? Can you show that it is *uncountable*? Consider Cantor's diagonalisation argument.)
